...
- 从每日拍卖中收集到的 ETH 的 30% 将分配给 veXNF 持有者：这与他们的 veXNF 权力成正比。选择领取 ETH 奖励的用户将收取 25% 的领取费用；然后，这笔费用将根据其 veXNF 功率重新分配给在一个周期的第一个小时内参与回收的参与者。
...
- 从每日拍卖中收集的 ETH 的 60% 将用于在市场外回购 XNF，特别是通过 Uniswap V3 上的 WETH-XNF 货币对。重要的是，回购的 XNF 将 100% 被销毁。此外，每个每日周期回收的 ETH 的 90% 将用于通过 Uniswap 上相同的 WETH-XNF 交易对从市场上回购 XNF，所有回购的 XNF 也将被销毁。
...
- 为促进 ETH 协议拥有的流动性持续增长，从第二个周期开始，60% 的 ETH（来自销毁费用、原生费用、互换费用，90% 来自回收）将被再投资于扩大协议拥有的流动性流动性。在此模型中，收集到的全部 ETH 都将用于市场购买并销毁 XNF。同时，等量的WETH被导入协议拥有的池中，从而增加了WETH-XNF流动性池中的WETH储备，并确保了整个范围的广泛支持。
...
- 在我们的协议中，从每日拍卖中收集的 60% 的 ETH 以及每个每日周期中回收的 ETH 的 90% 用于通过 Uniswap V3 上的 WETH-XNF 货币对直接从市场购买 XNF。随后，所获得的 XNF 100% 立即通过不可篡改的过程被销毁。这一过程确保了回购和销毁机制的完整性，消除了第三方参与的需要，从而减轻了与交易对手抢先交易相关的风险。
...
此外，Xenify 还采用了巧妙的刻录机制，使用户能够刻录 vXEN 和 YSL 代币来铸造 XNF。 vXEN 燃烧过程充当战略性第 1 层燃烧器，这是令人兴奋的部分 – 60% 用于执行 XNF 的自动回购和燃烧，30% 的燃烧费用返回给 veXNF 持有者，剩余的 10%发送给 Xenify 团队。这种方法不仅鼓励用户的长期参与，而且提高了可持续性，同时提高了对那些深入参与该协议的人的奖励。
...
对于每批销毁，您提供的代币（vXEN 或 YSL）将被永久从流通中删除。您的销毁费用中，60% 用于通过 XNF 的购买和销毁机制创建协议拥有的流动性，30% 作为原生奖励分配给 veXNF 持有者，其余 10% 发送给 Xenify 团队。这种销毁机制不仅仅是减少 XEN 和 YSL 代币的流通供应量；它体现了一种战略方法，将 YSL 和所有链上更广泛的 XEN 社区统一起来，同时支持我们生态系统的发展并奖励积极的参与者。
...
除了交换费用之外，部署了 XNF 的链上的 veXNF 持有者还将在每个周期结束时赚取燃烧费用产生的原生代币。由于 XNF 采用减半模型，并且每日 50% 的铸币保留给那些销毁 vXEN 或 YSL 的人，因此参与每日销毁拍卖是确保大量 XNF 头寸的少数手段之一。随着越来越多的用户燃烧 vXEN 和 YSL 来获取 XNF，veXNF 持有者不仅可以通过燃烧费用从增加的奖励中获利，而且生态系统也可以通过 XNF 的购买和燃烧机制，通过协议拥有的流动性的增加而受益。这种创新的燃烧周期实施创造了另一个积极的反馈循环，确保整个生态系统通过用户的积极参与蓬勃发展。
...
其工作原理如下：当用户在 Xenify 上发起代币兑换时，他们可以选择支付可选的 0.5% 兑换费用。这笔费用经过战略性分配，以鼓励未来的增长并激励用户参与。在部署了 XNF 的链上，交换费用的 60% 将用于通过 XNF 的购买和销毁机制来扩散协议拥有的流动性，30% 将作为原生奖励分配给 veXNF 持有者，其余 10% % 被团队用来支持 Xenify 的持续开发。
...
通过将批次价值与原生代币价格保持一致并提供批量折扣，我们的销毁费用公式在鼓励销毁的同时实现了透明度。此外，我们很自豪地强调，100％的销毁费用被重新部署，以通过 XNF 的购买和销毁机制增强协议拥有的流动性，支持 Xenify 的持续发展，并奖励 veXNF 持有者。这种方法不仅提高了效率，而且认可了社区成员的宝贵贡献，同时建立了一个互惠互利的生态系统，促进流动性的持续扩散。
...
这种回收机制为生态系统和回收者带来了多种好处。它允许用户参与每日销毁拍卖，而无需支付强制销毁代币，同时促进 XNF 代币的稀缺性。所有回收奖励的一部分用于通过 XNF 的购买和销毁机制产生协议拥有的流动性。该策略使用户能够以支持协议增长的方式最大化其收入。
...
一旦收取销毁费用，60% 将用于通过 XNF 的购买和销毁机制创建协议拥有的流动性。此外，该费用的 30% 会在周期结束时分配给 veXNF 持有者，而团队则收到剩余的 10%。从您的奖励中，将分配 0.5025 ETH 的折扣金额，其中 90%（0.45225 ETH）通过相同的销毁机制贡献给 XNF 协议拥有的流动性，10%（0.05025 ETH）将分配给 Xenify 团队。通过为 100 批次刻录做出贡献，您将在周期结束时收到 XNF。此外，通过在周期的第一个小时内回收奖励，您可以获得 25% 索赔费用中累积的 ETH 的一部分。

![Grey V4](https://user-images.githubusercontent.com/60996729/235287926-6b18081e-ca41-48c7-8dfc-29cc32c598f1.png)

## ⭐️ 本地代币奖励分配

在每个周期中，协议的奖励池逐渐积累由燃烧费用、交换费用和原生费用产生的原生代币。该池随着每笔交易而增长，无论是交换还是销毁。然后，奖励池的总金额根据 veXNF 持有者的 XNF 持有量及其相关的盈利能力分配给 veXNF 持有者。如果在周期结束时没有 veXNF 持有者，则交换费用、销毁费用和原生费用产生的原生奖励将用于通过 XNF 的购买和销毁机制创建协议拥有的流动性。每当用户参与一个没有 veXNF 持有者且所有锁已过期的循环时，就会触发该函数。

除了燃烧费用、交换费用和原生费用产生的奖励之外，该协议还将执行原生代币的独家分配，该原生代币源自 veXNF 持有者领取原生奖励时收取的 25% 领取费用。这种独家分配将保留给在周期的第一个小时内回收奖励的 veXNF 持有者。通过调整激励措施并将未使用的奖励投入使用，Xenify 创建了一个繁荣、可持续的生态系统，鼓励参与并培养不断增长的用户群，有助于巩固协议的长期增长。
...
这 1,500,000 个 XNF 代币将在发布当天作为锁定代币进行初始分配，并在 730 天内逐步解锁。例如，如果您从空投中收到 7300 个锁定的 XNF 代币，则预计每天您的钱包中将解锁大约 10 个 XNF 代币。由于这些代币将根据发布前的每日交换贡献分配给交换参与者，因此您在活动期间交换的越多，您获得的 XNF 空投份额就越大！现在，我们了解您渴望从这些 XNF 代币中获利，并见证您的投资组合蓬勃发展。然而，有一个小问题——空投的 XNF 代币在您的钱包中解锁之前无法进行质押。因此，如果您的目标是获得协议原生奖励的一部分，我们强烈建议您也参加初始燃烧拍卖。这是您从初始分配中赚取部分原生奖励的门票！
...
在 Xenify，安全对于确保我们协议的最大安全性和可靠性至关重要。为了加强我们协议的安全性，我们对业内备受推崇的安全公司 PeckShield 和 Solidproof 的合同进行了审计。我们的协议经过精心设计，以去中心化和自治为指导原则。 Xenify 的智能合约没有管理密钥，这意味着它们无法升级或由集中机构管理。加强我们协议真正的去中心化和自治性质。

随着 Xenify 的发展，我们的目标是优先考虑用户的利益，促进协议内的透明度和信任。 Xenify 的使命是通过在我们的所有努力中坚持加密货币的第一原则来创建一个透明、公平和以用户为中心的 DeFi 协议。通过坚持这些基本价值观，我们努力构建一个体现 DeFi 领域信任、公平和社区驱动发展的协议。
...
## ◽️ 合约是开源的吗？
- 我们的合同可供公开查看，但未获得许可。未经所有联合创始人明确书面同意，严禁全部或部分重新分发、修改或重新利用我们的合同。批准请求必须发送到联合创始人的官方电子邮件地址，并从这些地址接收直接回复。这些限制是为了保护我们持有人的利益。

![Grey V4](https://user-images.githubusercontent.com/60996729/235287926-6b18081e-ca41-48c7-8dfc-29cc32c598f1.png)

## ◽️ XNF 是否有初始供应？
  - 不，Xenify 没有初始供应，也没有 XNF 代币的预开采。相反，该协议每天都会奖励用户积极参与生态系统。这是通过各种贡献来实现的，例如通过直接燃烧 vXEN 或 YSL 代币进行的燃烧费用贡献、原生代币贡献以及通过 Xenify 聚合器生成的交换费用。值得注意的是，在受支持的 EVM 链上收取的 30% 的销毁费用、互换费用和原生费用每天都直接面向 veXNF 持有者，提供了巨大的长期盈利潜力。
...
- 此外，通过选择回收您的奖励，您将积极为 Xenify 生态系统的增长和可持续发展做出贡献。从回收的奖励中扣除销毁费用后，协议将剩余奖励的 90% 分配给通过 XNF 购买和销毁机制创建协议拥有的流动性，从而促进代币的稀缺性和更有效的市场。剩余的 10% 支付给团队，用于支持持续的运营费用，例如平台开发、营销活动和社区参与活动。
...
## ◽️ 如果我现在锁定 XNF，何时能收到奖励？
  ...
  - 这种衰减机制会激励用户锁定更长的期限，以更长时间地保持更高的盈利能力。在每个周期中，燃烧费用、交换费用和原生费用产生的奖励会累积在奖励池中，并且协议会在周期结束时根据 veXNF 持有者锁定的 XNF 代币和它与盈利能力相关，确保公平和透明的分配。
...
- 如果由于所有锁定到期而在一个周期结束时没有 veXNF 持有者，则协议的设计将重新定向 30% 的原生奖励（最初是针对 veXNF 持有者），以增加协议拥有的流动性。因此，整个分配通过在该周期内使用 XNF 的购买和销毁机制来促进协议的流动性。
