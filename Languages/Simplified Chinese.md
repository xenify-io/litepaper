...
- Xenify 将在其运行的每条链上发起空投活动。在此阶段，将向参与者分发 1,500,000 XNF。

- 对于 BNB 链和 Arbitrum，我们为期 60 天的空投活动将根据每个参与者通过交换费用的贡献分配 25,000 个 XNF 代币。这扩展到在所有支持的 EVM 兼容链上进行的交换，确保广泛的包容性。要获得空投资格，交易必须源自 EVM 链或在 EVM 链上完成，前提是参与者选择支付 0.5% 的交换费。

- 空投活动结束后，接收者将能够在部署后领取其 XNF 代币。这些空投代币将在730天内逐步解锁。例如，如果您从空投中领取了 7,300 个锁定的 XNF 代币，则预计每天将有大约 10 个 XNF 代币解锁到您的钱包中。
...
- **分配 1,500,000 XNF 作为独家空投：** 为了鼓励参与，每个链在部署前将获得 1,500,000 XNF 的独家空投。参与者将能够在发布第一天领取空投，并将以锁定代币的形式分发，在 730 天内逐步解锁。
...
隆重推出 Xenify 的独家 XNF 空投活动，该计划旨在推动跨多个 EVM 链的平台用户参与度。为了建立对即将在新链上发布 XNF 的预期，在部署之前将举办空投活动。每次活动期间，参与者将获得 1,500,000 个 XNF 代币的慷慨分配，这提供了从 Xenify 的增长中受益的绝佳机会。

这 1,500,000 个 XNF 代币将在发布当天作为锁定代币进行初始分配，并在 730 天内逐步解锁。例如，如果您从空投中收到 7300 个锁定的 XNF 代币，则预计每天您的钱包中将解锁大约 10 个 XNF 代币。由于这些代币将根据参与者在发布前的每日贡献进行分配，因此您在活动期间参与的越多，您获得的 XNF 空投份额就越大！现在，我们了解您渴望从这些 XNF 代币中获利，并见证您的投资组合蓬勃发展。然而，有一个小问题——空投的 XNF 代币在您的钱包中解锁之前无法进行质押。因此，如果您的目标是获得协议原生奖励的一部分，我们强烈建议您也参加初始燃烧拍卖。这是您从初始分配中赚取部分原生奖励的门票！
...
## ◽️ 如果每日拍卖没有参与者怎么办？
- 万一某个周期在 Xenify 上没有任何交换且没有用户参与销毁拍卖或本机拍卖的情况下通过，则不会发生每日 XNF 周期分配。 本周期内分配的 XNF 代币将不会被铸造。 因此，XNF 的总供应量将减少该周期内打算分配的数量。 例如，如果一个周期的每日分配量设置为 20,000 XNF，并且该周期内没有进行任何交换，也没有任何代币参与销毁和本地拍卖，那么这 20,000 XNF 将不会被铸造或分配。 相反，XNF 的总供应量将在周期结束时减少 20,000 XNF。

![Grey V4](https://user-images.githubusercontent.com/60996729/235287926-6b18081e-ca41-48c7-8dfc-29cc32c598f1.png)

## ◽️ 如果每日拍卖中只有一两次有参与者怎么办？
- XNF 分配通过每日三次拍卖进行。 每天，50% 的 XNF 代币被分配给销毁拍卖参与者。 此外，每日 XNF 供应量的 50% 保留用于 veXNF 奖励，这些奖励在分配给原生拍卖 (45%) 和掉期拍卖 (5%) 的参与者之前被锁定一年。 如果每日拍卖中的一两次缺乏参与者，协议将调整每日 XNF 分配以确保公平性。
  
- 该协议旨在跨不同场景动态重新分配奖励：在不销毁拍卖参与者的情况下，每日 XNF 的 90% 作为 veXNF 奖励分配给本地拍卖参与者，剩余 10% 作为 veXNF 奖励交换拍卖参与者； 在没有掉期拍卖参与者的情况下，50% 照常分配给销毁拍卖参与者，本地拍卖参与者将获得剩余的 50% 作为 veXNF 奖励。 如果没有原生拍卖参与者，则照常分配 50% 用于销毁拍卖参与者，剩余 50% 作为 veXNF 奖励用于交换拍卖参与者。 在本地和销毁拍卖参与者均不存在的情况下，100％作为 veXNF 分配给交换拍卖参与者； 没有原生和掉期拍卖参与者意味着销毁拍卖参与者 100%，并且没有掉期和销毁拍卖参与者将 100% 分配给原生拍卖参与者作为 veXNF 奖励。
  
- 简而言之 - 无论哪次拍卖，参与者都会获得奖励。 这种适应性方法确保即使在拍卖参与度较低的情况下，活动也能得到一致的奖励，从而促进平衡和高效的分配。
