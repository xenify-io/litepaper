...
- 此外，ETH 拍卖的参与者将获得 40% 的 XNF 供应量作为 veXNF 奖励，并附带 1 年可领取的锁定期。一旦领取，这些奖励就会增强参与者的赚钱能力，使他们能够赚取更多的 ETH。同样，参与互换拍卖的人将获得每日 XNF 分配的剩余 10% 作为 veXNF 奖励，同样具有 1 年可领取锁定期。领取后，他们的赚钱能力也会增加。
...
- 每日拍卖中收集的 ETH 的 50% 将用于通过 Uniswap V3 上的 WETH-XNF 交易对从市场上购买 XNF，所获得的 XNF 100% 将被销毁。

- 每个日周期回收的 ETH 的 90% 将用于通过 Uniswap 上的 WETH-XNF 交易对从市场上购买 XNF，所购买的 XNF 100% 将被销毁。

- 我们协议中的销毁机制经过精心设计，可在每次每日拍卖或回收参与后自动、不可变地执行，从而消除交易对手风险并减轻如果需要第三方发起回购和交易则可能出现的抢先交易的潜在问题。燃烧过程。
...
- 该协议最初将利用 Uniswap V3 来提供流动性。为了启动 WETH-XNF 交易对，它将分配 100,000 XNF，从而确定每个 XNF 1.00 美元的初始价格。值得注意的是，该价格可能会由协议进行调整，具体取决于初始周期期间收集的 ETH 数量。此后，将在 1% 的费用等级上创建全系列配对。

- 该协议还旨在确保 ETH 协议拥有的流动性在每个后续周期中永久增长。从第二个周期开始，每天通过销毁费用、原生费用、交换费用和回收活动获得的 ETH 将用于支持 XNF 流动性扩散。在此框架内，100% 的 ETH 用于市场购买和销毁 XNF。同时，用于回购和销毁 XNF 的等量 WETH 被分配到协议拥有的池中。这一行动导致 WETH-XNF 流动性池内的 WETH 大幅增加，确保全面覆盖整个协议拥有的范围。
...
- XNF 的最大供应量为 2260 万枚，这些代币的分配通过每日四次拍卖进行。每天，5,250,000 XNF（占 XNF 代币的 25%）被分配给参与 YSL 销毁拍卖的参与者。还向参与 vXEN 销毁拍卖的参与者支付相同数量的 5,250,000 XNF。此外，每日 XNF 供应量的额外 50% 用于 veXNF 奖励。但需要注意的是，这些奖励有一年的锁定期，之后将分发给参与 ETH 拍卖（40%）和掉期拍卖（10%）的个人。总的来说，这些拍卖每天分配 10,500,000 XNF。
