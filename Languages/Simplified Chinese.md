...
Xenify 采用战略方法每天分配 XNF 代币，重点奖励其运营的所有 EVM 链生态系统中的三个关键参与者群体。这些群体是通过每日铸造代币总数的预定百分比来识别的：50% 授予燃烧 vXEN 代币或 YSL 代币的用户，45% 授予本机代币贡献者，最后 5% 授予参与代币交换的用户。

例如，每日铸造量为 20,000 个 XNF 代币，考虑到可变的百分比指定，默认情况下分配不会平均分配。因此，vXEN 和 YSL 代币燃烧器获得了 10,000 个 XNF 代币，占 50% 的份额。原生代币贡献者被分配 9,000 个 XNF 代币，相当于分配的 45%，而交换参与者则被授予 1,000 个 XNF 代币，占剩余的 5%。这种系统化的分配流程在所有 EVM 链上都是统一的，确保对不同的用户活动提供公平和比例的奖励。
...
该协议通过考虑分配的 XNF 代币总量 (TD)、周期内销毁的批次总数 (TB) 以及您在周期内销毁的批次数量，计算分配给销毁费用贡献的每日 XNF 份额。循环（BB）。以下是详细解释的 vXEN 刻录分配 (BA) 的公式：
...
<table>
    <tr>
        <td><strong>BA</strong><br>（销毁分配）</td>
        <td>特定用户根据一个周期内销毁的批次获得的 XNF 数量。</td>
</tr>
    <tr>
    ...
    <tr>
        ...
        <td>正在计算燃烧贡献的特定用户在一个周期内燃烧的批次数。</td>
</tr>
    <tr>
        <td><strong>TB</strong><br>（总批次）</td>
        <td>一个周期内所有用户燃烧的批次总数。</td>
</tr>
</table>

<br>

其工作原理如下：协议首先将计划在周期内分配的 XNF 代币总量 (TD) 乘以 50% 来计算用户的销毁分配 (BA)，因为每日 50% 的 XNF 铸币分配给了销毁的参与者vXEN 或 YSL。然后，它计算特定用户销毁的批次 (BB) 与所有用户销毁的总批次 (TB) 的比例，从而得出每个批次销毁的值。最后，协议将这些值相乘，以确定用户在该周期 (BA) 的个人燃烧分配。
...
让我们通过一个示例来说明回收功能的工作原理。想象一下，您是 Arbitrum 上的 veXNF 持有者，您决定在周期的第一个小时内回收您的本机链奖励余额 1.0 ETH。首先，协议计算您的燃烧贡献。由于 Arbitrum 上的批次值为 0.01 ETH，因此您的 1.0 ETH 奖励相当于 100 个批次。接下来，根据费用公式从奖励中扣除您的燃烧费用。第 1 小时回收意味着您的销毁费用为 0.4975 ETH。
...
- XNF 代币不会预先开采或分配给投资者或团队成员。相反，用户可以通过积极参与协议来获取 XNF 代币，无论是通过燃烧 vXEN 或 YSL 代币、参与本机拍卖还是从支持的链交换代币。这种方法奖励用户的贡献，并保证 XNF 在生态系统内的公平分配。
...
- YSL 被设计为一种超通缩代币，通过 Xenify 的每日销毁拍卖将可量化的稀缺性和实用性融为一体。凭借固定的总供应量和内置的销毁机制，随着循环供应量的减少，YSL 的价值有可能随着时间的推移而升值。 YSL 通过与 Xenify 的每日销毁拍卖相集成来实现这一目标，该拍卖会将新铸造的 XNF 分配给选择销毁其 YSL 代币的用户。与其他仅依赖交易活动的通货紧缩代币不同，这种持续不断的每日销毁 YSL 代币的需求来源会产生永久的通货紧缩压力。简而言之，YSL 的通货紧缩代币经济提供了一个有趣的机会——具有编码的稀缺性、实用性和可预测的价值增长驱动力。
