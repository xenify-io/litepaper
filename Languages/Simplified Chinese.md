...
其工作原理如下：当用户在 Xenify 上发起代币兑换时，他们可以选择支付可选的 0.5% 兑换费用。这笔费用经过战略性分配，以鼓励未来的增长并激励用户参与。在部署了 XNF 的链上，交换费用的 70% 将用于通过 XNF 的购买和销毁机制来扩散协议拥有的流动性，25% 将作为原生奖励分配给 veXNF 持有者，其余 5% % 被团队用来支持 Xenify 的持续开发。
...
一旦收取销毁费用，70% 将用于通过 XNF 的购买和销毁机制创建协议拥有的流动性。此外，该费用的 25% 会在周期结束时分配给 veXNF 持有者，而团队则收到剩余的 5%。从您的奖励中，将分配 0.5025 ETH 的折扣金额，其中 90%（0.45225 ETH）通过相同的销毁机制贡献给 XNF 协议拥有的流动性，10%（0.05025 ETH）将分配给 Xenify 团队。通过为 100 批次刻录做出贡献，您将在周期结束时收到 XNF。此外，通过在周期的第一个小时内回收奖励，您可以获得 25% 索赔费用中累积的 ETH 的一部分。
...
<p align="center" dir="auto">
  <picture>
    <source media="(prefers-color-scheme: light)" srcset="https://github.com/xenify-io/litepaper/assets/66044824/27fc3a21-5f1f-42e5-b120-33cb18bbbd4b" class="source-light">
    <source media="(prefers-color-scheme: dark)" srcset="https://github.com/xenify-io/litepaper/assets/66044824/706469c2-4f97-4fff-9cb3-72a139ed3719" class="source-dark">
...
## ◽️ XNF 是否有初始供应？
  - 不，Xenify 没有初始供应，也没有 XNF 代币的预开采。相反，该协议每天都会奖励用户积极参与生态系统。这是通过各种贡献来实现的，例如通过直接燃烧 vXEN 或 YSL 代币进行的燃烧费用贡献、原生代币贡献以及通过 Xenify 聚合器生成的交换费用。值得注意的是，在受支持的 EVM 链上收取的 25% 的销毁费用、互换费用和原生费用每天都直接面向 veXNF 持有者，提供了巨大的长期盈利潜力。
...
- 如果由于所有锁定到期而在一个周期结束时没有 veXNF 持有者，则协议的设计将重新定向 25% 的原生奖励（最初是针对 veXNF 持有者），以增加协议拥有的流动性。因此，整个分配通过在该周期内使用 XNF 的购买和销毁机制来促进协议的流动性。
