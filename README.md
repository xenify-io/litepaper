...
At the core of Xenify's bold vision is a cutting-edge liquidity aggregation protocol that streamlines cross-chain token swaps. Whether you want to swap tokens on Ethereum, BNB Chain, Polygon, Arbitrum, or any of the 50 supported chains, Xenify has you covered. With access to 26 different aggregators, Xenify guarantees unparalleled liquidity and optimal rates for your swaps. By leveraging cutting-edge APIs, Xenify bridges seamless communication with a wide array of DEX aggregators, dramatically enhancing the swapping experience and streamlining the quest for optimal swap routes across the vast tapestry of chains.
...
To participate in Xenify's daily burn auction, users need to have native chain tokens in their wallet to cover both the Mint Fee and Burn Fee. The specific Mint Fee required depends on the current market value of the tokens being burned. When users provide the Mint Fee, 100% of the native chain tokens are sent to the Mint Fee Manager. 

The Mint Fee Manager will utilise the native chain tokens to create protocol-owned liquidity, which also serves to provide additional XNF and native XEN tokens rewards for recyclers at each cycle's end. The resulting mechanism ensures that users who engage in the burn auction not only bolster protocol-owned liquidity but indirectly generate additional rewards for recyclers as well. This has been intentionally designed to help ensure the sustainability and long-term growth of the Xenify ecosystem. Plus, with no admin keys on the contract where the protocol-owned liquidity is held, any possibility of manipulation is eliminated.
...
## ◽️ Xenify
  ...
  - Xenify is a decentralised finance (DeFi) protocol designed to revolutionise the token swap experience. It offers a state-of-the-art cross-chain aggregator, making it easy to swap tokens across 50 different chains. With access to the best routes and lowest fees from 26 aggregators, Xenify empowers users to seamlessly swap tokens between supported chains. It emphasises fairness, simplicity, transparency, user empowerment, and interoperability, aiming to create a more connected and user-friendly DeFi ecosystem.
