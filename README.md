...
XEN is a cryptocurrency designed by the Fair Crypto Foundation to achieve the original mission of blockchain, focusing on mass market adoption with the lowest barrier to entry compared to other coins on the market today. The project aims to connect like-minded people together and solve problems in the crypto world, such as overbought and undiscovered cryptocurrencies.

Key features of XEN include its simplicity, being based on the ERC20 token standard with minimal code, no pre-minting, hidden doors, admin keys, or origin wallets. Participants can mint their own XEN using their Ethereum-compatible wallets without depositing any funds. XEN starts with zero supply and will only be minted by the participating community. It has no maximum cap on the supply, making it inflationary in the beginning and disinflationary as adoption increases. XEN is a digital asset with no backing by investors or intrinsic value and is immutable, meaning it cannot be changed or stopped by anyone.

XEN operates on a Proof of Participation (PoP) system, allowing participants to mint their own coins. The XEN smart contract uses a fair system of new token distribution based on reward time locks and the total number of participants. XEN's value is linked to its difficulty to be minted, similar to Bitcoin. As more participants join and mint XEN, the total amount of generated XEN drops, making it more scarce and valuable. The only way to mint more XEN in the future is by extending the time one has to wait to receive the mint.

Participants can mint XEN by connecting their Ethereum-compatible wallets to the XEN smart contract and specifying the number of days they are willing to wait to receive XEN. The smart contract will generate a XEN rank (cRANK) based on how many people interacted with the contract before the participant. XEN can be staked for 1 to 1,000 days with APY rewards starting at 20% on XEN Genesis and decreasing by 1 percentage point every 90 days until it reaches 2%, where it will stay indefinitely. Each stake's APY is fixed at the time of the stake, depending on how many days have passed since XEN Genesis.
...
Xenify's minting process is structured to reward users who contribute to the protocol's growth by participating in various activities. This approach promotes an equitable distribution of XNF tokens, thereby fostering a decentralized and community-driven environment.
...
### ♦️ Swapping 
  - Experience the capabilities of Xenify, a pioneering cross-chain aggregator of aggregators, and enjoy seamless token exchanges on the platform. Your swapping activities contribute to the platform's growth, simultaneously increasing your equity in the protocol through daily XNF token distributions. Furthermore, you benefit from the efficiency and competitive rates offered by aggregated routes across multiple aggregators, which add a layer of convenience to your cross-chain transactions. As such, you're not just a user, but an active participant shaping the future of this dynamic ecosystem.
...
Using the Xenify protocol to swap tokens incurs a fee, primarily aimed at maintaining the rewards pool for XNF stakers. When a user engages in a token swap on Xenify, they are charged a swap fee that amounts to 0.5% of the total swap value. This fee serves a dual purpose: it rewards the XNF stakers for their involvement and investment in the ecosystem, and it supports the ongoing growth and development of the protocol.
...
<p align="center" dir="auto">
  <picture>
    <source media="(prefers-color-scheme: light)" srcset="https://github.com/xenify-io/litepaper/assets/60996729/efba081b-157a-4d05-8abd-18893cbfd60a" class="source-light">
    <source media="(prefers-color-scheme: dark)" srcset="https://github.com/xenify-io/litepaper/assets/60996729/efba081b-157a-4d05-8abd-18893cbfd60a" class="source-dark">
    <img alt="Xenify" src="https://github.com/xenify-io/litepaper/assets/60996729/efba081b-157a-4d05-8abd-18893cbfd60a" style="visibility:visible;max-width:100%;">
  </picture>
</p>
