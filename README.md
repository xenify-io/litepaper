...
- [Deployment Schedule](https://github.com/xenify-io/litepaper/blob/main/README.md#%EF%B8%8F-deployment-schedule)
- [XNF Liquidity (Uniswap V3 Integration)](https://github.com/xenify-io/litepaper/edit/main/README.md#%EF%B8%8F-xnf-liquidity-uniswap-v3-integration)
- [XNF Minting Process and Incentives](https://github.com/xenify-io/litepaper/blob/main/README.md#%EF%B8%8F-xnf-minting-process-and-incentives)
...
Introducing vXEN, Xenify's innovative solution crafted to simplify participation in its multi-chain burn auctions. By burning XEN tokens on the native chain, this initiative highlights Xenify's steadfast dedication to advancing interoperability across multiple chains, reinforcing its stature as a leading layer-1 burner of XEN.

To acquire vXEN, users must first hold native XEN tokens. Should a user choose to convert their XEN into vXEN, the “Burn” function of the XEN token contract on Ethereum is activated. This action results in the XEN being permanently burnt, with the user's address captured in the burn record for the emerging XEN layer 1 token (X1). Once this on-chain procedure is finalised, a quantity of vXEN, aligned with the chain's set ratio, is produced on the user's selected chain.

With vXEN tokens in their possession, holders can opt to either participate in the daily burn auction on a specific chain or effortlessly transfer their vXEN to any other chain where XNF is operational. For instance, on the BNB Chain, a burn ratio of 200:1 means that a conversion of 200 bXEN yields 1 vXEN. Holders have the flexibility to either bridge their current vXEN tokens or generate new ones by burning native XEN. This setup empowers vXEN holders to engage in Xenify's burn auctions, be it on the BNB Chain or any other chain that Xenify supports.

For optimal cross-chain bridging of vXEN, Xenify has chosen LayerZero OFT as its primary bridging protocol. Should any unexpected challenges arise, Axelar and Wormhole are on standby as backup bridging solutions. By utilising an array of specialised, decentralised protocols, Xenify guarantees steadfast interoperability between chains.
...
Xenify offers users the convenience of burning vXEN or YSL tokens directly from their wallets, facilitating participation in the daily burn cycle. To secure XNF using vXEN, users must burn native XEN tokens, receiving vXEN in return. Notably, securing one batch necessitates a consistent 10,000,000 vXEN. For example, if you're keen on joining Xenify's burn auction on Arbitrum, you'll need to burn a minimum of 10,000,000 vXEN, and in return, you'll obtain 1 batch. This burn procedure underscores our protocol's significance in layer-1 cross-chain burning of XEN tokens. Each XEN token burn ensures your address is recorded in the X1 burn record, with vXEN granted in return.

It's crucial to understand that burning your vXEN to partake in the daily burn auction for XNF allocation incurs a Burn Fee. Upon cycle completion, you'll be allocated XNF tokens proportional to the vXEN batches you've burned. Similarly, burning YSL tokens also attracts a Burn Fee. The fixed conversion rate during the YSL burn process equates 0.1 YSL to one batch. At the cycle's end, XNF tokens are allocated based on the YSL batches you've burned.

Whether you choose to burn vXEN or YSL, the value per batch (VB) hinges on your chosen burn chain. This value, determined at Xenify’s deployment, dynamically reflects the current price of the chain’s native token. The total value of your burned batches directly influences the Burn Fee (BF) needed for the burn function, ensuring transparency and smoothness for participants.

For every batch burned, the supplied tokens (vXEN or YSL) are irrevocably removed from circulation. Of your Burn Fee, 40% contributes to protocol-owned liquidity for XNF, 40% is shared as native rewards among veXNF holders, 10% funds an automated daily XNF buyback, and the final 10% goes to the Xenify team. This burn strategy transcends mere reduction of XEN and YSL circulating supply. It represents a unified approach, bridging the YSL and wider XEN communities across chains, fostering our ecosystem's growth, and rewarding engaged participants.
...
Our platform is thoughtfully designed with decentralisation and autonomy as guiding principles. Xenify's smart contracts are devoid of admin keys, meaning they cannot be upgraded or governed by a centralised authority. To reinforce our protocol's truly decentralised and autonomous nature, earning potential will be equitably distributed among participants based on their veXNF holdings and its associated power. 

As Xenify evolves, we aim to prioritise our users' interests, nurturing transparency and trust throughout the protocol. Xenify's mission is to forge a transparent, equitable, and user-centric DeFi platform by upholding the First Principles of Crypto in all our endeavours. By committing to these core values, we aspire to establish a protocol that champions trust, fairness, and community-driven growth in the DeFi space.
